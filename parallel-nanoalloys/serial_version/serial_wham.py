...
class WHAM:
	...
	def save_data(self, input_file_name, output_filename, temp_start, temp_end, num_data_points=100, convergence_epsilon=1e-3, convergence_max_iter=100):
		...
		U_2_estimate = np.empty(num_data_points)
		U_estimate = np.empty(num_data_points)

		for i, log_beta in enumerate(beta_range):
			print(i)
			U_estimate[i] = float(self.__A_estimate(U, log_beta).array[0])
			U_2_estimate[i] = float(self.__A_estimate(U_2, log_beta).array[0])

		c_v = (U_2_estimate - U_estimate**2) / (N * k_B * T_range**2)  # now just in [eV / K]
		c_v = (c_v / k_B)  # nondimensionalized c_v

		frame = pd.DataFrame(columns=["temp", "c_v", "U_estimate", "U^2_estimate"])
		frame["temp"] = T_range
		frame["c_v"] = c_v
		frame["U_estimate"] = U_estimate
		frame["U^2_estimate"] = U_2_estimate
